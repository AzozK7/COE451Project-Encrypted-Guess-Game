...
from Crypto.Cipher import AES								# Module for AES Encryption and Decryption	
from Crypto.Util.Padding import pad,unpad

#	Connection initialization

s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)			# initialize Socket for the connection
s.bind(("127.0.0.1",9000))										# Bind ip and port for the server to use
s.listen(5)
...
def AEScipherENC(key,PlainText,IVrecievedDECODED):							# AES encrypton Function
	...
	print(Fore.GREEN + Ptext + Style.RESET_ALL) 
	Ctext =  "[+] Cipher Text (After) ==> "+base64.b64encode(CipherText).decode('utf-8')+"\n"
	print(Fore.GREEN + Ctext + Style.RESET_ALL) 							# Print the corrosponding Ciphertext
...
def AEScipherDECr(key,CT,IVrecievedDECODED):								# AES decryption Function
	...
	Ptext = "[+] Plain Text (After) ==> "+PlainText+"\n"
	Ctext =  "\n[+] Cipher Text (Before) ==> "+base64.b64encode(CT).decode('utf-8')
	print(Fore.GREEN + Ctext + Style.RESET_ALL)   
	print(Fore.GREEN + Ptext + Style.RESET_ALL)   											# Print the corrosponding Plaintext
	return PlainText			
